const getTemplate = (placeholder, data, selectedId) => {
    ...
    const items = data.map(item => {
        let cls = ''
        if (item.id === parseFloat(selectedId)) {
            text = item.value
            cls = 'selected';
        }

        return `
            <li id="${item.id}" class="select__item ${cls}" data-type="item" data-id="${item.id}">${item.value}</li>
        `
    })
    return `
...
export class Select {
    ...
    get currentSelect() {
        return this.options.data.find(item => item.id === parseFloat(this.selectedId));
    }

    select(id) {
        this.selectedId = id;
        this.$value.textContent = this.currentSelect.value;
